...
- `ParallelDegree` — Max parallelism for rendering; `null` or <= 0 selects a heuristic (typically `Environment.ProcessorCount`).
...
Max parallelism for rendering; `null` or <= 0 selects a heuristic (typically `Environment.ProcessorCount`).
